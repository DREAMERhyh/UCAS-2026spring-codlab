...
wire intr_flush;         
wire pipe_flush;         
wire [31:0] pipe_jump_target;

// ====================== 中断相关寄存器 ======================
reg [31:0] mepc;         // 机器异常程序计数器
reg        intr_enable;  // 全局中断使能
...
always @(posedge clk) begin
    if (rst) begin
        ...
        if (Inst_Req_Valid && !Inst_Req_Ready) begin
            ...
            redirect_target <= pipe_jump_target;
        end
...
wire stale_inst = (discard_cnt > 0) || (pipe_flush && Inst_Valid) || (redirect_pending && Inst_Valid);
...
assign Inst_Req_Valid = inst_req_holding
                        || (!rst
                            && !inst_buf_valid
                            && !req_accepted
                            && !stall_if_id);
...
// ================== 中断响应与控制 ==================
assign intr_flush = intr_enable && intr && !rst && !redirect_pending;

always @(posedge clk) begin
    if (rst) begin
        mepc <= 32'h0;
    end else if (intr_flush) begin
        mepc <= pc_reg;
    end
end
...
always @(posedge clk) begin
    if (rst) begin
        intr_enable <= 1'b1;   // 修正：复位后中断默认使能
    end else if (intr_flush) begin
        intr_enable <= 1'b0;   // 响应中断，屏蔽后续中断
    end else if (idex_is_MRET && idex_valid) begin
        intr_enable <= 1'b1;   // 执行MRET，恢复中断使能
    end
end

// 总跳转目标：中断优先级高于分支跳转
assign pipe_jump_target = intr_flush ? 32'h100 : ex_branch_target;
assign pipe_flush = branch_flush | intr_flush;

// PC 更新逻辑
always @(posedge clk) begin
    if (rst)
        pc_reg <= 32'h0;
    else if (pipe_flush && !(Inst_Req_Valid && !Inst_Req_Ready))
        pc_reg <= pipe_jump_target;
...
wire is_MRET   = is_SYSTEM && (funct3 == 3'b000) && (funct7 == 7'b0011000) && (rs2 == 5'b00010);
...
wire        id_is_MRET   = is_MRET && ifid_valid;
...
wire branch_ge_unsigned = ~branch_lt_unsigned;
...
// 分支与冲刷控制信号，加入MRET触发跳转
assign branch_flush = (idex_Jump || (idex_Branch && branch_taken) || idex_is_MRET) && idex_valid;
assign flush_if_id = pipe_flush;
assign flush_id_ex = pipe_flush;
